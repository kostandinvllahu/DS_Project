...
ArrayList<Integer> duplicate = new ArrayList<Integer>(); //Krijohet nje arraylist qe te rruaj te gjitha duplikatat me shume se nje
    ...
    if(key == root.key){ 
        root.count++;
       //root.right = insertNode(root.right, key); //rjesht kodi per testim
        return root;
    }
...
//    }
    ...
    //ndiq inorder tree
        ...
        if(root.key == value && root.count > 1){
           ...
           // if(value == root.key){
           //Ky rrjesht kodi shton count duplicaten
            root.count++;
    ...
    */
    ...
    public void replaceNode(Node root, int value, int replaceWith){
        ...
        if(root.key == value && root.count > 1){
            ...
            //Kjo eshte nqs dua te zevendosej vetem duplikatat
           // root.count = 0;
           //======KY RRJESHT KODI BEN FUNSKIONIN E FUTJES DUPLIKATE===========
            for(int i = 0; i< count -1; i++){
            insert(replaceWith);
            }
            //==========MBARON KETU=============
